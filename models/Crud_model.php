...
class Crud_model extends CI_Model
{
    ...
    public function super_stockist()
    {
        $query = $this->db->select('*')->from('country')
            ->join('state', 'country.country_id = state.country_id')
            ->join('city', 'state.id = city.state_id')
            ->join('super_stockist_details', 'city.id = super_stockist_details.city')
            ->get()->result();
        return $query;
    }
    ...
    public function distributor()
    {
        $query = $this->db->select('*')
            ...
            ->join('state', 'country.country_id = state.country_id')
            ->join('city', 'state.id = city.state_id')
            ->join('distributor_details', 'city.id = distributor_details.city_id')
            ->get()->result();
        return $query;
    }
